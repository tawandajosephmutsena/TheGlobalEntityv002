...
The palette is a vibrant collision of tropical energy and earthy foundations. We use the Material Design tokens to create a sophisticated, layered environment.

### The Palette
*   **Primary (#af047a):** Our "Passion Magenta." Used for high-impact CTAs and signature storytelling moments.
...
*   **The Layering Principle:** Use the spacing scale (e.g., `spacing-8`) and background color shifts to separate content. High-priority content "floats" by being the lightest color (`surface-container-lowest`) on a slightly darker base.
...
*   **Primary:** Rounded `full` (9999px). Background is a gradient of `primary` to `primary-dim`. No border.
