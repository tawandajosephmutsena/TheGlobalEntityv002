        ...
        return (
            ...
            >
                <div className="relative">
                    ...
                    <div className="relative aspect-[16/9] sm:aspect-[21/9] bg-muted overflow-hidden">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500 scale-90 group-hover:scale-100">
                            <div className="size-16 rounded-full bg-primary/95 backdrop-blur-xl flex items-center justify-center text-primary-foreground shadow-[0_0_50px_rgba(var(--primary),0.3)] border border-white/20">
                        ...
                        <div className="absolute inset-x-6 top-6 flex justify-between items-start pointer-events-none">
                            <div className="flex gap-2">
                                <div className="px-3 py-1.5 rounded-full bg-black/40 text-white text-[9px] font-black tracking-widest backdrop-blur-xl border border-white/10 flex items-center gap-2 animate-in fade-in slide-in-from-top-4 duration-700">
                                    {podcast.media_type === 'video' ? <Video className="size-3" /> : <Headphones className="size-3" />}
                                    {podcast.media_type}
                                </div>
                            </div>
                            <div className="px-3 py-1.5 rounded-full bg-primary text-primary-foreground text-[9px] font-black tracking-widest shadow-lg shadow-primary/20 animate-in fade-in slide-in-from-top-4 duration-700 delay-100">
                                Featured
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="p-6 md:p-8 space-y-4 relative">
                        <div className="flex flex-wrap items-center gap-4">
                            ...
                            <span className="text-[10px] font-black tracking-[0.2em] text-muted-foreground/50">
                                {displayDate}
                            </span>
                        </div>

                        <h3 className="text-2xl md:text-3xl font-black leading-[1.1] tracking-tight group-hover:text-primary transition-colors duration-500 line-clamp-1">
                            {podcast.title}
                        </h3>

                        {podcast.description && (
                            <p className="text-base text-muted-foreground leading-relaxed line-clamp-1 opacity-70 group-hover:opacity-90 transition-opacity duration-500">
                                {podcast.description}
                            </p>
                        )}

                        <div className="flex items-center gap-6 pt-3 border-t border-border/40">
                            <span className="flex items-center gap-2 text-[10px] font-black tracking-widest text-muted-foreground/60 group-hover:text-foreground transition-colors">
                                <Play className="size-3.5 text-primary fill-current opacity-40" />
                                {podcast.play_count.toLocaleString()} Plays
                            </span>
                            <span className="flex items-center gap-2 text-[10px] font-black tracking-widest text-muted-foreground/60">
                                <Clock className="size-3.5 text-primary opacity-40" />
